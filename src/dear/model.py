...
class ResEncoder(nn.Module):
    ...
    '''
    def __init__(self, latent_dim=64, arch='resnet', dist='gaussian', fc_size=2048, noise_dim=128, pretrained= False):
        super().__init__()
        ...
        if arch == 'resnet':
            self.encoder = resnet50(
              pretrained=pretrained, in_channels=in_channels, fc_size=fc_size, out_dim=out_dim
              )
        else:
...
class BGM(nn.Module):
    ...
    def __init__(self, latent_dim=64, conv_dim=32, image_size=64,
                 ...
                 prior='gaussian', num_label=None, A=None, pretrained= False):
        super().__init__()
        self.latent_dim = latent_dim
        ...
        self.encoder = ResEncoder(latent_dim, enc_arch, enc_dist, enc_fc_size, enc_noise_dim, pretrained= pretrained)
