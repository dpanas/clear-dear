...
def make_dataloader(args):
    ...
    if args.dataset == 'celeba':
        ...
        train_set = dataload_withlabel( 
            args.data_dir, image_size= args.image_size, label_file= args.label_file, mode= '', sup_prop= args.sup_prop
        )
...
class dataload_withlabel(torch.utils.data.Dataset):
    def __init__( self, root, label_file=None, image_size=64, mode="train", sup_prop=1., num_sample=0):
        ...
        if label_file is not None:
            if label_file == 'labels.csv':
                label_path = os.path.join( self.root, label_file)
                print( label_path)
                self.attrs_df = pd.read_csv( label_path)
                
                self.img_paths = self.attrs_df['image'].values
                
                self.labels = self.attrs_df.drop('image', axis=1).values
                ...
                np.random.seed(2)
                self.available_label_index = np.random.choice(self.n, int(self.n * sup_prop), replace=False)

            else:
                self.attrs_df = pd.read_csv(os.path.join( self.root, label_file))
                self.split_df = pd.read_csv(os.path.join( self.root, label_file))
                ...
                for i in range(3):
                    ...
                    root1 = self.root + mode1
                    imgs = os.listdir(root1)
                    self.img_paths += [os.path.join( self.root, mode1, k) for k in imgs]
                self.img_paths = np.array(self.img_paths)[mask]
        else:
            root = self.root + mode
            imgs = os.listdir(root)
            self.img_paths = [os.path.join(root, k) for k in imgs]
            try:
                # that was originally in DEAR for the 'pendulum' dataset, that has labels in image names
                self.imglabel = [list(map(float, k[:-4].split("_")[1:])) for k in imgs]
            except Exception:
                self.imglabel = [[-1,] for i in range(len(imgs))]
    ...
    def __getitem__(self, idx):
        if self.label_file == 'labels.csv':
            img_path = os.path.join( self.root, 'images', self.img_paths[idx])  
           # print(f"Loading image from: {img_path}")  
            label = self.labels[idx]

            if idx not in self.available_label_index:
                label = torch.zeros_like(torch.tensor(label)) - 1  
        ...
        return data, label.float(), img_path
